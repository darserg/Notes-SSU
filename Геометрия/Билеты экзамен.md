...
3. *Признак коллинеарности векторов в координатах*
   ***Теорема***:
...
3. *Определение смешанного произведения, формула вычисления в координатах*
	***Определение***:
	Смешанным произведением векторов $\vec{a}, \vec{b} \text{ и } \vec{c}$ определяется следующим выражением: $\vec{a}\vec{b}\vec{c}=[\vec{a},\vec{b}]\vec{c}$
...
3. Применение смешанного произведения
   ...
   2. Определение ориентации 
    ...
    3. Вычисление объёмов многогранников: $V_{тет}=\frac{1}{6}V_{пар}=\frac{1}{6}|\vec{a}\vec{b}\vec{c}|$
       $$
       V_{тет}=\frac{1}{6}mod\begin{vmatrix}
a_{x} & a_{y} & a_{z} \\
b_{x} & b_{y} & b_{z} \\
c_{x} & c_{y} & c_{z}
\end{vmatrix}
...
$$
\begin{array}{l}
\text{Имеем: } V_{тетр} = \frac{1}{6} |\overline{AB}\cdot\overline{AC}\cdot\overline{AD}|  \\
...
3. *Теорема об ортогональных инвариантах фигур $II$-го порядка на плоскости*
...
\to \text{две прямые скрещиваются тогда и только тогда, когда}: \\
\begin{vmatrix}
x_{2}-x_{1} && y_{2}-y_{1} && z_{2} - z_{1} \\
a_{x} && a_{y} && a_{z} \\
b_{x} && b_{y} && b_{z}
...
2. *Уравнение касательной плоскости к конусу второго порядка*
   $$
   \alpha: \frac{x_{0}}{a^2}x + \frac{y_{0}}{b^2}y - \frac{z_{0}}{c^2}z = 0
   $$
   <u>Вывод:</u>
   $$
   \begin{gathered}
