...
***Определение*** (Гейне):
$f(x)$ называется непрерывной в точке $a$, если $\forall \epsilon > 0 \space \exists \delta_{\epsilon} > 0:$ $\forall x\in A:$ $|x-a|<\delta_{\epsilon}$ выполняется $f(x) \to_{n \to \infty} f(a)$
